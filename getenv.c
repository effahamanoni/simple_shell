 ...
 *
 * Return: A pointer to a string array containing the environment.
 */
...
int _setenv(info_t *info, char *var, char *value)
{
	...
	_strcpy(buf, var), _strcat(buf, "="), _strcat(buf, value);
	node = info->env;
	...
	while (node)
	{
		...
		if (p && *p == '=')
		{
			free(node->str), node->str = buf, info->env_changed = 1;
			return (0);
		}
		node = node->next;
	}
	add_node_end(&(info->env), buf, 0), free(buf), info->env_changed = 1;
	return (0);
}
